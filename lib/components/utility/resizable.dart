...
export "../common/wrappers.dart";

part "resizable/resize_mode.dart";
part "resizable/model.dart";

class ResizableComponent extends WebComponent {
  ResizableModel model = new ResizableModel();
  
  Point _previousPoint;
  
  DivElement get _wrapper => this.query(".x-resizable_ui-wrapper");
  Element get _content => _wrapper.query("*");
  Element get _ghostElement => _wrapper.query(".x-resizable_ghost");
  DivElement get _rightHandle => _wrapper.query(".x-resizable_ui-resizable-e");
  DivElement get _bottomHandle => _wrapper.query(".x-resizable_ui-resizable-s");
  DivElement get _bottomRightHandle => _wrapper.query(".x-resizable_ui-resizable-se");
  ...
  void inserted() {
    ...
    ghostElement.classes.add("x-resizable_ghost");
    _wrapper.children.insert(1, ghostElement);
    _refreshGhost();
    
    model.currentSize.width = _content.clientWidth;
    model.currentSize.height = _content.clientHeight;
    _refreshView();
    model.ratio = model.currentSize.width / model.currentSize.height;
    
    document.body.onMouseMove.listen(_onResizing);
    document.body.onMouseUp.listen(_onResizeStopped);
  }
  
  bool get aspectratio => model.aspectRatio;
  set aspectratio(var aspectratio) {
  ...
  void _refreshGhost() {
    if (_ghostElement != null) {
      if (!model.ghost) {
        _ghostElement.classes.add("x-resizable_ui-helper-hidden");
      }
      else {
        _ghostElement.classes.remove("x-resizable_ui-helper-hidden");
      }
    }
  }
