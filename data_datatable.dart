...
@CustomTag('datatable-page')
class DatatablePage extends PolymerElement {
  ...
  DatatablePage.created() : super.created();
  
  @override
  void enteredView() {
    super.enteredView();
    
    this.shadowRoot.querySelectorAll('h-datatable').forEach((DatatableComponent datatable) {
      datatable.on['sorted'].listen((CustomEvent event) {
        print('sorted: ${event.detail}');
      });
      
      datatable.on['filtered'].listen((Event event) {
        print('filtered');
      });
      
      datatable.on['selected'].listen((Event event) {
        print('selected');
      });
      
      datatable.on['edited'].listen((Event event) {
        print('edited');
      });
      
      datatable.on['columnResized'].listen((Event event) {
        print('column resized');
      });
      
      datatable.on['rowToggled'].listen((Event event) {
        print('row toggled');
      });
    });
  }
